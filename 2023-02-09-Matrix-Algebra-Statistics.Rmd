...
Empirical version of covariance is
$$
s_{jk}=\frac{1}{n}\sum_{i=1}^{n}(x_ij-\bar{x_j})(x_{ik}-\bar{x_k})
$$
For small n, say $n \le 20$, we should replace the factor $\frac{1}{n}$ by $\frac{1}{n-1}$ to correct for a small bias.
...
$$
\rho_{jk}=\frac{v_{jk}}{\sqrt{var(\vec{x_j})}\sqrt{var(\vec{x_k})}}
$$
...
**Note**  
Correlations and covariances measure only linear dependence. The quadratic dependence of $Y = X^2$ on $X$ is not reflected by these measures of dependence. If $x,y$ are independent, then $\rho_{xy}=v_{xy}=0$; but the converse is not true as examplified by the case of quadratic dependence.
